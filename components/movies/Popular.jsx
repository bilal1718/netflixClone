...
const Popular = ({AddtoList}) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`https://api.themoviedb.org/3/movie/popular?api_key=58eec0999b7db5c6b801fecec14d2b51`);
    ...
    return (
        <>
            <h1 className='bg-zinc-900 text-white font-extrabold text-4xl p-4'>Popular Movies</h1>
                ...
                <div className="modal-overlay fixed top-10 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50">
                    <div className="modal-container max-w-md mx-auto bg-white rounded-lg overflow-hidden shadow-lg p-6">
                        ...
                        <div className="flex justify-center space-x-4">
                            ...
                            <button className="bg-gray-700 text-white px-4 py-2 mb-4 rounded-full hover:bg-gray-800 
                            focus:outline-none focus:ring 
                            focus:border-gray-500" onClick={() => AddtoList(selectedMovie)}>Add to List</button>
                        </div>
                    </div>
                </div>
            )}
